...
//! hand. `cotrex script <file>` runs it through rtk and then shows `git diff` — the change is verified
//! from the diff, not by re-reading every file. cotrex itself never generates the script (the agent
...
Scripts/ is ready. For a repetitive or multi-file change, don't edit files one by one:
  ...
  2. Run it:  cotrex script Scripts/<name>.sh
cotrex runs the script through rtk, then shows `git diff` — verify the change from the diff instead of
